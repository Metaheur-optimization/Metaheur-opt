...
Results.AllBestFitnesses=zeros(Out.NRun, 1);
...
%     NFEAll=NFEAll+Npopulation;     %????
    ...
    for iIter=1:Out.MaxIter % Maximum number of iterations (itermax)
        ...
        for i=1:Npopulation
            r=rand;
            if r>AP
                Xnew(i,:)= Position(i,:)+fl*rand(1, NDecisionVariable).*(PositionMemory(num(i),:)-Position(i,:)); % Generation of a new position for crow i (state 1)
               % Farshid just added this. it is checking feasibility of new
               % positions
                while all(Xnew(i,:))<all(LowerBound(:)) || all(Xnew(i,:))>all(UpperBound(:))
                     Xnew(i,:)= Position(i,:)+fl*rand(1, NDecisionVariable).*(PositionMemory(num(i),:)-Position(i,:));
                end
...
%                 while Xnew(i,j)<LowerBound(j) || Xnew(i,j)>UpperBound(j)
%                      Xnew(i, j) =  LowerBound(j)+(UpperBound(j)-LowerBound(j)).*rand;
%                 end
%             end
            %%%%%%%%%%%%%%%%%%%%%%%%
            Bool=1;
...
%             end
        ...
        Bestfit(iIter)=min(min(fitnessMemory)); % Best fitness found at iteration t
    ...
    Results.AllBestFitnesses(iRun, 1)=min(Bestfit);
% Results.NFEAll=NFEAll;
% Results.PenaltyCount=PenaltyCountAll;
end
Results.NFEAll=NFEAll;
Results.PenaltyCount=PenaltyCountAll;
best_ = min(min(Results.AllBestFitnesses))
mean_ = mean(Results.AllBestFitnesses)
std_ = std(Results.AllBestFitnesses)
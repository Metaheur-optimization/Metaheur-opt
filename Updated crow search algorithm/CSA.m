...
%     Position(1,:)=[0.0516890284000, 0.3567169544000, 11.2890117993000];
    ...
    for iIter=1:Out.MaxIter % Maximum number of iterations (itermax)
        ...
        BestGlobalMemory=find(fitnessMemory== min(fitnessMemory));     %The best Global memory during all iterations
        [Ms ds]=sort(fitnessMemory,'descend');                         %Sorting Memories accordig to their fitness
        final_per=2;                                                   
     kbest=final_per+(1-iIter/Out.MaxIter)*(100-final_per);         %Percentage of the best crows at the last iteration
     kbest=round(Npopulation*kbest/100);
        for i=1:Npopulation
            if rand>AP
     dif=zeros(kbest,NDecisionVariable);
        for j=1:kbest
            k=ds(j);
        if i~=k
            dif(j,:)=fl*rand*(PositionMemory(k,:)-Position(i,:));
        end
        end
        
        difference=sum(dif)/kbest;
        XXnew(i,:)= Position(i,:)+difference;          %New positions at the first stage of mutation
